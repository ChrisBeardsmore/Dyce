...
if uploaded_file:
    ...
    for duration in [12, 24, 36]:
        ...
        column_config[f"Standing Charge (Base {duration}m)"] = st.column_config.NumberColumn(
            f"Standing Charge (Base {duration}m)", format="%.2f", disabled=True, help="Base rate from supplier (p/day)"
        )
        column_config[f"Unit Rate (Base {duration}m)"] = st.column_config.NumberColumn(
            f"Unit Rate (Base {duration}m)", format="%.3f", disabled=True, help="Base rate from supplier (p/kWh)"
        )
        column_config[f"Sell Standing Charge ({duration}m)"] = st.column_config.NumberColumn(
            f"Sell Standing Charge ({duration}m)", format="%.2f", disabled=True, help="Base + Uplift (p/day)"
        )
        column_config[f"Sell Unit Rate ({duration}m)"] = st.column_config.NumberColumn(
            f"Sell Unit Rate ({duration}m)", format="%.3f", disabled=True, help="Base + Uplift (p/kWh)"
        )
    ...
    if st.checkbox("Show Debug Info"):
        if not st.session_state.input_df.empty:
            ...
            st.write(f"Unit Uplift (12m): {test_row.get('Unit Rate (Uplift 12m)', 'NOT FOUND')}")
            st.write(f"Base Unit (12m): {test_row.get('Unit Rate (Base 12m)', 'NOT FOUND')}")
            st.write(f"Final Unit (12m): {test_row.get('Sell Unit Rate (12m)', 'NOT FOUND')}")
